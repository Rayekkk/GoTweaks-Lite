...
{
    /// <summary>
    /// Property for Legion Y2 button remap (JSON ButtonMapping format).
    /// Not auto-bound to UI - call SendMapping() explicitly to send to helper.
    /// </summary>
    internal class LegionButtonY2Property : WidgetControlProperty<string, ComboBox>
    {
        public LegionButtonY2Property(ComboBox inUI, Page inOwner) : base("", Function.LegionButtonY2, inUI, inOwner)
        {
        ...
        {
            if (json == null) return;
            // force bypasses the json==Value dedup for cold-start recovery (see Y1 property).
            if (force)
            {
